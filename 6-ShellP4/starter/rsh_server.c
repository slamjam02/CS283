...
int boot_server(char *ifaces, int port){
    ...
    // bind
    RDSH_DEF_CLI_CONNECT.sin_family = AF_INET;
    RDSH_DEF_CLI_CONNECT.sin_addr.s_addr = INADDR_ANY; // Accept connections on any IP
    RDSH_DEF_CLI_CONNECT.sin_port = htons(RDSH_DEF_PORT); // Convert port to network byte order

    int bind = bind(svr_socket, (struct sockaddr*)&server_addr, sizeof(server_addr));
    if (bind < 0) {
        perror("bind");
        close(svr_socket);
        return ERR_RDSH_COMMUNICATION;
